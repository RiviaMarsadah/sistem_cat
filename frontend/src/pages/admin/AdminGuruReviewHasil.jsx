...
import '../guru/ReviewHasil.css';

export default function AdminGuruReviewHasil() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/guru-data/rekap/detail/${id}`);
  ...
  return (
    <div className="review-hasil-page" style={{ padding: '20px' }}>
      <div className="review-container">
        <div className="review-header-compact">
          <div className="header-left">
            <button className="btn-back-minimal" onClick={() => navigate(-1)} style={{ cursor: 'pointer' }}>
              <FiArrowLeft /> Kembali
            </button>
            <div className="student-info-main">
              ...
              <div>
                ...
                <p className="student-meta-small">NIS: {data.siswa?.nis} • Kelas {data.siswa?.kelas?.tingkat} {data.siswa?.kelas?.jurusan?.namaProdi || 'N/A'}</p>
              </div>
            </div>
          </div>
        ...
        <div className="exam-info-strip">
          <FiInfo /> 
          <span><strong>{data.jadwalUjian?.nama}</strong> • {data.jadwalUjian?.mataPelajaran?.namaMapel} • {data.totalSoal} Soal • Guru: {data.jadwalUjian?.guru?.user?.namaLengkap || '-'}</span>
        </div>
                ...
                <div className="q-body-compact">
                  ...
                  {j.bankSoal?.gambar && (
                    <div className="q-image-small">
                      <img src={`${BASE_URL}/uploads/${j.bankSoal.gambar}`} alt="Soal" onError={(e) => { e.target.style.display = 'none'; }} />
                    </div>
                  )}
